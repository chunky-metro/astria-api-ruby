...
describe Astria::Client do
  ...
  describe "initialization" do
    ...
    it "defaults :base_url to production API" do
      ...
      expect(subject.base_url).to eq("https://api.astria.com/")
    end
  end
  ...
  describe "authentication" do
    it "uses HTTP authentication if there's a password provided" do
      ...
      expect(WebMock).to have_requested(:get, "https://api.astria.com/test").with(basic_auth: %w(user pass))
    end
    ...
    it "uses access token if there's an access token provided" do
      ...
      expect(WebMock).to(have_requested(:get, "https://api.astria.com/test")
  ...
  describe "#execute" do
    ...
    it "raises RequestError in case of error with a JSON response" do
      stub_request(:post, %r{/foo}).to_return(read_http_fixture("transferDomain/error-inastria.http"))

      expect {
        subject.execute(:post, "foo", {})
      }.to raise_error(Astria::RequestError, "The domain google.com is already in Astria and cannot be added") do |exception|
    ...
    it "raises a Request error in case of an error with a JSON response with attribute errors" do
      ...
      expect {
        subject.execute(:post, "foo", {})
      }.to raise_error(Astria::RequestError, "Validation failed") do |exception|
    ...
    it "raises RequestError in case of error with an HTML response" do
      ...
      }.to raise_error(Astria::RequestError, "502 Bad Gateway") do |exception|
        expect(exception.attribute_errors).to be_nil
      end
    end
    ...
    it "raises RequestError in absence of content types" do
      ...
      }.to raise_error(Astria::RequestError, "405 Method Not Allowed") do |exception|
        expect(exception.attribute_errors).to be_nil
      end
    end
  end
  ...
  describe "#request" do
    ...
    it "performs a request" do
      ...
      expect(WebMock).to have_requested(:get, "https://api.astria.com/foo")
          .with(basic_auth: %w(user pass),
                headers: { 'Accept' => 'application/json', 'User-Agent' => Astria::Default::USER_AGENT })
    end
      ...
      allow(HTTParty).to receive(:put)
          .with(
              "#{subject.base_url}foo", {
                ...
                query: { foo: "bar" },
                basic_auth: { username: "user", password: "pass" },
                headers: { 'Accept' => 'application/json', 'Content-Type' => 'application/json', 'User-Agent' => Astria::Default::USER_AGENT, "Custom" => "Header" },
              }
            )
      ...
      allow(HTTParty).to receive(:post)
          .with(
              "#{subject.base_url}foo", {
                ...
                body: { something: "else" },
                basic_auth: { username: "user", password: "pass" },
                headers: { 'Accept' => 'application/json', 'Content-Type' => 'application/x-www-form-urlencoded', 'User-Agent' => Astria::Default::USER_AGENT },
              }
            )
    ...
    it "supports custom user agent" do
      allow(HTTParty).to receive(:get)
          .with(
              "#{subject.base_url}test",
              { format: :json,
                headers: hash_including("User-Agent" => "customAgent #{Astria::Default::USER_AGENT}"), }
            )
